...
public class PieDial : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    private RectTransform baseRect; // The clickable circle
    ...
    [Tooltip("When true, the dot is clamped to the dial radius while dragging.")] [SerializeField]
    private bool clampDotToRadius = true;

    [Tooltip(
        "Minimum normalized magnitude required to confirm on release (0..1 of radius). 1.0 means must cross the edge.")]
    [Range(0f, 1.5f)]
    [SerializeField]
    private float confirmMagnitudeNormalized = 1.0f; // 1.0 == at or beyond edge

    [Tooltip("Snap the resulting direction to N sectors (e.g., 8 for 8-way). Use 0 for no snapping.")] [SerializeField]
...
public void OnPointerUp(PointerEventData eventData)
    {
        ...
        Vector2 localPoint;
        if (!RectTransformUtility.ScreenPointToLocalPointInRectangle(baseRect, eventData.position, _uiCamera,
                out localPoint))
        {
            HideIndicators();
        ...
        float finalNormalizedMag = _radius > 0f ? mag / _radius : 0f;

        bool confirmed = finalNormalizedMag >= confirmMagnitudeNormalized;
        if (confirmed)
        {
...
private void UpdateDrag(PointerEventData eventData)
    {
        ...
        // Move dot to show drag direction and magnitude
        if (dotRect != null)
        {
            ...
            // Always show the dot position relative to center
            dotRect.anchoredPosition = target;
            
            // Scale dot based on magnitude to show confirm threshold
            float dotNormalizedMag = _radius > 0f ? mag / _radius : 0f;
            float dotScale = Mathf.Lerp(0.8f, 1.2f, Mathf.Clamp01(dotNormalizedMag / confirmMagnitudeNormalized));
...
private void ShowIndicators()
    {
        ...
        if (dotRect != null)
        {
            ...
            dotRect.anchoredPosition = Vector2.zero; // Start at center (player position)
        }
    }
...
private void HideIndicators()
    {
        ...
        if (dotRect != null) 
        {
            ...
            dotRect.localScale = Vector3.one; // Reset scale
        }
    }
